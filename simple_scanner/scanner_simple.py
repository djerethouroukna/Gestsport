#!/usr/bin/env python3
# ==============================================================================
# VERSION SIMPLIFIÉE DU SCANNER GESTSPORT
# ==============================================================================
...
class SimpleScannerApp:
    """Application simplifiée du scanner"""
    
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("🎯 Scanner GestSport - Simple")
    ...
    def setup_ui(self):
        ...
        tk.Label(scan_frame, text="Positionnez un QR code devant la caméra",
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get('success'):
                    ...
                    self.show_result("REJETÉ", result.get('message', 'Ticket invalide'), ticket_number)
    ...
    def show_result(self, statut, message, ticket_number):
        ...
        if statut == "VALIDÉ":
            self.result_label.configure(text=f"✅ {statut}: {message}", fg='#27ae60')
        elif statut == "REJETÉ":
            self.result_label.configure(text=f"❌ {statut}: {message}", fg='#e74c3c')
        elif statut == "HORS LIGNE":
            self.result_label.configure(text=f"🔴 {statut}: {message}", fg='#e67e22')
        else:
            self.result_label.configure(text=f"⚠️ {statut}: {message}", fg='#f39c12')
    ...
    def manual_scan(self):
        """Scan manuel"""
        ticket_number = simpledialog.askstring("Scan Manuel", "Entrez le numéro du ticket:")
        if ticket_number:
...
def main():
    ...
    try:
        ...
        logger.info("Demarrage scanner simple")
        app.root.mainloop()
