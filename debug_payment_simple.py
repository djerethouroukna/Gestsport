#!/usr/bin/env python3
"""
Script de diagnostic simple pour comprendre pourquoi le statut de paiement reste "non payé"
"""
...
class PaymentStatusDebugger:
    ...
    def check_recent_reservations(self):
        """Verifie les reservations recentes et leur statut de paiement"""
        print("\n--- Reservations recentes (dernieres 24h): ---")
        ...
        if not recent_reservations:
            print("   Aucune reservation recente trouvee")
            return
    ...
    def check_pending_payments(self):
        ...
        for payment in pending_payments:
            ...
            # Verifier la reservation associee
            res = payment.reservation
            print(f"      Reservation statut paiement: {res.payment_status}")
    
    def check_stripe_config(self):
        """Verifie la configuration Stripe"""
        ...
        # Verifier si les cles sont en mode test
    ...
    def simulate_webhook_test(self):
        ...
        # Trouver une reservation avec paiement en attente
        pending_payment = Payment.objects.filter(status='pending').first()
        
        if not pending_payment:
            print("   Aucun paiement en attente a tester")
            return
        
        print(f"   Test avec paiement #{pending_payment.id}")
        
        # Simuler la mise a jour manuelle
        try:
            ...
            print(f"   Simulation reussie:")
            print(f"      Paiement #{pending_payment.id} mis a jour: paid")
            print(f"      Reservation #{reservation.id} mise a jour: paid")
            
        except Exception as e:
            print(f"   Erreur simulation: {e}")
    
    def check_webhook_url(self):
        """Verifie l'URL du webhook"""
        print("\n--- URL du Webhook: ---")
        ...
        try:
            ...
            print(f"   URL complete: http://127.0.0.1:8000/payments/stripe/webhook/")
    ...
    def run_diagnostic(self):
        ...
        print("=== Demarrage du diagnostic complet ===")
        ...
        print("1. Verifiez que le webhook Stripe est configure dans votre dashboard Stripe")
        print("2. L'URL du webhook doit etre: https://votreserveur.com/payments/stripe/webhook/")
        print("3. Le webhook secret doit correspondre a celui dans settings.py")
        print("4. Assurez-vous que le serveur est accessible publiquement pour les webhooks")
        print("5. Verifiez les logs Django pour les erreurs de webhook")
        print("\n=== Si probleme persiste: ===")
        print("- Utilisez la simulation manuelle (ci-dessus) pour tester")
        print("- Verifiez les logs Stripe dans votre dashboard")
